...
// This library
import Common.{ TextFile, Schema, splitValidations }
...
object Migrations {
  ...
  implicit val schemaOrdering = implicitly[Order[Int]].contramap[Schema](_.self.version.addition)
   ...
   * @return
   */
  def distinctSchemas(schemas: List[Schema]): Map[RevisionCriterion, List[Schema]] =
    schemas.groupBy(_.revisionCriterion)
   ...
   */
  def buildMigration(sourceSchema: Schema, successiveSchemas: List[Schema]): Validation[String, RedshiftMigration] = {
    val flatSource = flattenJsonSchema(sourceSchema.data, splitProduct = false).map(_.elems)
    val flatSuccessive = successiveSchemas.map(s => flattenJsonSchema(s.data, splitProduct = false)).sequenceU
   ...
   */
  def mapSchemasToTargets(schemas: List[Schema]): List[(Schema, List[List[Schema]])] = {
   ...
   */
  def getOrdering(migrationMap: ValidMigrationMap): Map[RevisionCriterion, Validation[String, List[String]]] =
    migrationMap.filterKeys(_.version.addition == 0).map {
      case (description, Success(migrations)) =>
        ...
        (description.revisionCriterion, longestMigration.success)
      case (description, Failure(message)) =>
        (description.revisionCriterion, message.failure)
    }
   ...
   */
  def buildAdditionMigrations(schemas: List[Schema]): ValidMigrationMap = {
    val migrations = for {
   ...
   * @return migration map of each Schema to list of all available migrations
   */
  def buildMigrationMap(allSchemas: List[Schema]): ValidMigrationMap = {
    ...
    val groupedMigrationMap = distinctSchemas(allSchemas).map {
      case (criterion, schemas) => (criterion, buildAdditionMigrations(schemas))
    }
  ...
  /**
   * Transform whole [[ValidMigrationMap]] with different schemas, models and revisions

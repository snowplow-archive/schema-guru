...
To upload fatjar to S3 bucket called `guru-jar-bucket` you can use `upload` task:

```bash
$ inv upload guru-profile guru-bucket
```

To provision cluster and start job you need to use `run_emr` task:

```bash
$ inv run_emr guru-profile guru-bucket guru-bucket/errors/ guru-bucket/output/ guru-bucket/input/ spark-ec2-keypair subnet-3dc2bd2a
```

If you need some specific options for Spark job, you can specify it in ``tasks.py``.
Spark job accept same options as CLI application but `--output` isn't optional and we have optional `--errors-path`.
